...
interface IAddSceneObjectPlaneProps {
...
interface IAddSceneObjectPlaneState {
  readonly id: string
  readonly classId: number
  readonly localZOrder: number
...
export class AddSceneObject extends React.Component<IAddSceneObjectPlaneProps, IAddSceneObjectPlaneState> {
  constructor(props: IAddSceneObjectPlaneProps) {
    super(props)
  ...
  private onOK = () => {
    ...
    const localZOrder = this.state.localZOrder
    ...
    if (this.props.onOkCallback) {
      this.props.onOkCallback({ id, classId, localZOrder, bornTime, bornX, bornY, destinationX, destinationY })
    }
  ...
  private onLocalZOrderChanged = (event: any) => {
    this.setState({ localZOrder: Number(event.target.value) })
  }
  ...
  private buildLocalZOrderSelect() {
    let options: any[] = []

    this.props.manager
      .getGameObjectStore()
      .getObjectLevelTypes()
      .forEach((objectLevel, index) => {
        options.push(
          <option value={objectLevel.value} key={index + 1}>
            {objectLevel.name}
          </option>
        )
      })
    ...
    return (
      <FormGroup controlId="bullet">
        ...
        <Col sm={12 - smSize}>
          <FormControl componentClass="select" defaultValue="0" onChange={this.onLocalZOrderChanged}>
            {options}
          </FormControl>
        </Col>
      </FormGroup>
