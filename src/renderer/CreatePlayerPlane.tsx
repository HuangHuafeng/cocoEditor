...
interface ICreatePlayerPlaneProps {
  readonly manager: Manager
  readonly playerPlaneToEdit: number
  readonly onDismissed: () => void
}

interface ICreatePlayerPlaneState {
  readonly id: string
  readonly friendPlane: number
...
export class CreatePlayerPlane extends React.Component<ICreatePlayerPlaneProps, ICreatePlayerPlaneState> {
  private addOrEdit: string

  constructor(props: ICreatePlayerPlaneProps) {
    super(props)

    if (this.props.playerPlaneToEdit === 0) {
      this.state = {
        id: '0',
        friendPlane: 0,
        bornTime: '0',
  ...
  private onOK = () => {
    const id = 0
    const friendPlane = this.state.friendPlane
    ...
    this.props.manager.createPlayerPlane(id, friendPlane, bornTime, bornX, bornY, destinationX, destinationY)
  ...
  private onFriendPlaneChanged = (event: any) => {
    this.setState({ friendPlane: event.target.value })
  }
  ...
  private buildFriendPlaneSelect() {
    ...
    const allFriendPlanes = this.props.manager.getGameObjectStore().getAllFriendPlanes()
    allFriendPlanes.forEach((fp, index) => {
      options.push(
        <option value={fp.id} key={index + 1}>
          {fp.name}
        </option>
      )
    })
      ...
      <FormGroup controlId="bullet">
        ...
        <Col sm={12 - smSize}>
          <FormControl
            ...
            placeholder="select"
            defaultValue="0"
            onChange={this.onFriendPlaneChanged}
          >
            {options}
          </FormControl>
        </Col>
  ...
  public render() {
    return (
      <Modal show={true} onHide={this.props.onDismissed} backdrop="static">
        <Modal.Header>
          <Modal.Title>{this.addOrEdit == 'edit' ? '编辑友方飞机' : '新建友方飞机'}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form horizontal>
